...
class GetRecommendTutorsByUserIdUseCase @Inject constructor(
    ...
    operator fun invoke(userId: String) = flow<EDSResult<List<String>>> {
        try {
            ...
            val response = recommendCoursesRepository.getRecommendTutorsByUserId(userId)
            if (response.isSuccessful) {
                val list = response.body()!!.data
                Log.d("GetRecommendTutorsByUserIdUseCase", list.toString())
